...
async function main() {
  ...
  const missingTableDeclarations = findMissingTableDeclarations(
    astComments.keys(),
    runtimeMeta.keys(),
  );
  if (missingTableDeclarations.length > 0) {
    throw new Error(
      `Missing source declarations for runtime tables: ${missingTableDeclarations.join(", ")}. `
      + "Refusing to generate SQL that would clear their comments.",
    );
  }
